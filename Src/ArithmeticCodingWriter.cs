...
/// <remarks>
///     <para>
///         The writer accepts a sequence of symbols as inputs via <see cref="WriteSymbol(int)"/>, encodes them and writes the
///         encoded bytes to the specified stream. A symbol is an integer; the same integer will be returned by <see
///         cref="ArithmeticCodingReader.ReadSymbol"/> when the encoded stream is decoded. Symbols are encoded according to
///         the current <see cref="ArithmeticSymbolContext"/>, which describes the relative frequencies of all symbols.</para>
///     <para>
///         Symbols are permitted to have a frequency of zero, but it is illegal to attempt to encode a zero-frequency symbol.
///         The symbol context does not have to remain unchanged; the context may be modified arbitrarily between calls to
///         <see cref="WriteSymbol(int)"/>, and an entirely different context may be applied using <see cref="SetContext"/>.
///         The only requirement is that identical contexts are in place before every <see cref="WriteSymbol"/> call and
///         before its corresponding <see cref="ArithmeticCodingReader.ReadSymbol"/> call.</para>
///     <para>
...
///         context, you can dedicate an extra symbol (with a suitable frequency) to mark end of stream, or write total symbol
///         count to the stream separately.</para>
///     <para>
///         The reader and the writer support operation on a stream which has other data before and/or after the
///         arithmetic-coded section. The reader's <see cref="ArithmeticCodingReader.Finalize"/> method ensures that the input
///         stream is left with exactly the correct number of bytes consumed.</para>
///     <para>
...
///         Arithmetic coding is not data compression per se; it is an entropy coding algorithm. Arithmetic coding requires an
///         accurate prediction of each symbol's probability to be supplied by the caller in order to be effective, and is
///         only as good as the caller's modelling of the sequence of symbols being passed in.</para></remarks>
...
public class ArithmeticCodingWriter
{
    ...
    ///     The frequency of each symbol occurring. When reading the data back using an <see cref="ArithmeticCodingReader"/>,
    ///     the set of frequencies must be exactly the same.</param>
    /// <remarks>
    ...
    ///     applied context. Symbol contexts may be changed arbitrarily between calls to <see cref="WriteSymbol"/>, as long as
    ///     the same changes are made during decoding between calls to <see cref="ArithmeticCodingReader.ReadSymbol"/>.</remarks>
    ...
    /// <summary>
    ///     Finalizes the stream by flushing any remaining buffered data and writing the synchronization padding required by
    ///     the reader. This call is mandatory; the stream will not be readable in full if this method is not called. This
    ///     method does not write enough information to the stream for the reader to detect that there are no further symbols;
    ///     see Remarks on <see cref="ArithmeticCodingWriter"/> for further info.</summary>
