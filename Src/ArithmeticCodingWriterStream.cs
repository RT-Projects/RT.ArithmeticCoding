...
public class ArithmeticCodingWriterStream : Stream
{
    ...
    ///     the stream is closed). Must be 257 elements long. When reading the data back using an <see
    ...
    public ArithmeticCodingWriterStream(Stream outputStream, uint[] frequencies)
    ...
    ///     applied context. Symbol contexts may be changed arbitrarily between calls to <see cref="Write"/>, as long as the
    ///     same changes are made during decoding between calls to <see cref="ArithmeticCodingReaderStream.Read"/>.</remarks>
