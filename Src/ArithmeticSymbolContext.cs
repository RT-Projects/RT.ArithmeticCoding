...
public abstract class ArithmeticSymbolContext
{
    ...
    ///     return a value for every possible input: 0 for all symbols less than the first valid symbol, and a value equal to
    ///     <see cref="GetTotal"/> for all symbols greater than the last valid symbol.</summary>
    public abstract uint GetSymbolPosition(int symbol);

    /// <summary>
    ///     The maximum sum of all symbol frequencies. The encoder/decoder can overflow if the sum of all frequencies exceeds
    ///     this value.</summary>
    public const uint MaxTotal = 0x8000_0000;
}

/// <summary>
///     Implements a symbol context best suited for relatively small ranges of valid symbols. Supports efficient updates of
///     individual symbol frequencies. The smallest symbol supported is 0, and the memory usage goes up linearly with the
///     largest valid symbol.</summary>
public class ArithmeticSymbolArrayContext : ArithmeticSymbolContext
{
    ...
    /// <param name="length">
    ///     How many symbols to keep track of. Valid symbols are in the range of 0 .. <paramref name="length"/>. All symbols
    ///     outside of this range have a frequency of 0.</param>
    /// <param name="initializer">
    ///     A function which returns the initial value for each symbol's frequency. Optional; if omitted, every symbol starts
    ///     with a frequency of 1.</param>
    ...
    /// <param name="updater">
    ///     A method which receives the current array of symbol frequencies. This method can modify the array arbitrarily, but
    ///     cannot replace it with an entirely new array. To change the length of the frequencies array, see <see
    ...
    ///     cref="SetSymbolFrequency(int, uint)"/> to update a small number of frequencies more efficiently.</summary>
    /// <param name="updater">
    ///     A function which receives the current array of symbol frequencies. This function can modify the array arbitrarily
    ///     and return it, or construct and return an entirely different array.</param>
    ...
    /// <summary>
    ///     Updates the frequency of the specified symbol. To update a large number of frequencies in one go more efficiently,
    ///     use <see cref="UpdateFrequencies(Action{uint[]})"/>.</summary>
    ...
    ///     The value to be added to the current frequency. This can be negative. If the symbol frequency becomes negative, an
